(function() {
  ...
  Container = (function() {
    ...
    Container.prototype.getWithStack = function(id, stack, callback) {
      var diConfig, instantiated, required, resolvedAsParam, service, _ref;
      if (((function() {
      ...
      diConfig = this.getDiConfig(id);
      if (((_ref = diConfig.params) != null ? _ref[id] : void 0) != null) {
        return callback(diConfig.params[id]);
      }
      required = this.require(id);
      if (!(diConfig.call != null) && !(diConfig.instantiate != null) && !(diConfig.inject != null)) {
        return callback(required);
      }
      if (!(diConfig.inject != null)) {
        instantiated = this.instantiate(required, diConfig);
        if (!(diConfig.instanceCall != null)) {
          return callback(instantiated);
        }
        this.doCallback(instantiated, diConfig.instanceCall, function(service) {
          return callback(service);
        });
      } else {
        this.resolveInjection(id, diConfig.inject, stack, __bind(function() {
          var services;
          services = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
          return callback(this.instantiate.apply(this, [required, diConfig].concat(__slice.call(services))));
        }, this));
      }
      return;
    };
    Container.prototype.resolveInjection = function(id, items, stack, parentCallback) {
      var next, resolved;
      resolved = [];
    ...
    Container.prototype.doCallback = function(service, callback, complete) {
      return service[callback.call](function() {
        var results;
        results = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
        return complete(callback.callback.apply(callback, results));
      });
    };
